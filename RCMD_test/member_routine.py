...
# parameter order from php : user_num, length, part, timing, level

def main(user_num, length, bodypart, timing, level) :
	...
	if len(routine_vid_num_list) < 15 :
		...
		if not ac_users_vid_list :
			...
			video_similarity = dataProcessor.jaccard_similarity(ac_users_vid_list, routine_vid_num_list)
			top_video_list = dataProcessor.v2v_top_k_video(video_similarity, 15)

			MyDB.execute('select url from ROUTINE where video_num in ' + str(tuple(top_video_list)))
		...
		if not history_db :
			...
			# best case -> same len
			if len(routine_vid_num_list) != len(default_crosstab.columns.values) :

				# add not played video column to default_df, all value is zero.
				for i in routine_vid_num_list :
			...
			if user_num in default_crosstab.index.values :
...
if __name__ == '__main__' :
	...
	main(USER_NUM, LENGTH, BODYPART, TIMING, LEVEL)